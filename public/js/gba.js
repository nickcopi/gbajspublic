...
GameBoyAdvance.prototype.loadSavedataFromWeb = function(saveData) {
	this.setSavedata(this.base64ToArrayBuffer(saveData));
}

GameBoyAdvance.prototype.base64ToArrayBuffer=function (base64) {
...
GameBoyAdvance.prototype.serverSavedata = function(slot) {
	var sram = this.mmu.save;
	if (!sram) {
		this.WARN("No save data available");
		return null;
	...
	let data = new Object;
	data.game = ROM;
	data.data = this.encodeBase64(sram.view);
	data.slot = slot;
	request.open('POST', url, true);
	request.setRequestHeader('Content-type', 'application/json');
	request.onload = ()=>{
		if(request.response.error) console.log(request.response.error);
		loadSuccess();
	}
